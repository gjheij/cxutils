...
def set_threshold(name=None, borders=None, set_default=None):
    ...
    set a default value if you do not wish to set a certain threshold (usually this is the minimum/maximum of your array
    depending on what kind of thresholding you're about to do, e.g., 'greater/smaller than <value>').
...
class SurfaceCalc(object):
    ...
    This object does all the surface initialization given a subject and a freesurfer directory. For instance, it reads in the
    curvature, thickness, and sulcal depth maps from freesurfer, smooths the curvature map, reads by default the
    V1_exvivo_thresh label in, and creates a boolean mask of this label. So with this one class you will have everything you
    need from the surface calculations.
    ...
    Notes
    ----------
    Embedded in :class:`cxutils.optimal.CalcBestVertex`, so if you can also just call that class and you won't have to run the
    command in "usage"
    """
    ...
    def __init__(
        ...
        self.ctx_path = opj(cortex.database.default_filestore, self.subject)
        if os.environ.get("PROJECT") not in self.ctx_path:
            raise TypeError(f"""Project '{os.environ.get('PROJECT')}' not found in '{self.ctx_path}'. This can happen if you
                            changed the filestore, but haven't reloaded the kernel. Use 'call_ctxfilestore' to set the
                            filestore (and reload window if running from VSCode)""")
        ...
        self.lh_surf = cortex.polyutils.Surface(self.lh_surf_data[0], self.lh_surf_data[1])
        self.rh_surf = cortex.polyutils.Surface(self.rh_surf_data[0], self.rh_surf_data[1])
        ...
        if make_mask:
            pp = self.label_to_mask(
                subject=self.subject,
                lh_arr=getattr(self, f'lh_{self.roi_label}'),
                rh_arr=getattr(self, f'rh_{self.roi_label}'),
                hemi="both"
            )
    ...
    def smooth_surfs(self, kernel=10, nr_iter=1):
        ...
        smooth surfaces with a given kernel size. The kernel size does not refer to mm, but to a factor. Therefore it has to
        be an integer value
        ...
        setattr(
            ...
            'lh_surf_sm',
            self.lh_surf.smooth(
                self.curvature.data[:self.lh_surf_data[0].shape[0]],
                factor=kernel,
                iterations=nr_iter
            )
        )
    ...
    def read_fs_label(
        ...
        fs_label: str, optional
            ROI-name to extract the vertex from as per ROIs created with `FreeSurfer`. Default is V1_exvivo.thresh
        hemi: str, optional
            For which hemisphere to perform the process, `lh`=left hemisphere, `rh`=right hemisphere, `both`=both hemispheres
            (default = `both`)
    ...
    def label_to_mask(self, subject=None, lh_arr=None, rh_arr=None, hemi="both"):
        ...
        lh_arr: numpy.ndarray, optional
            array containing the mask in the left hemisphere (can be read from :class:`cxutils.optimal.SurfaceCalc` itself)
        rh_arr: numpy.ndarray, optional
            array containing the mask in the right hemisphere (can be read from :class:`cxutils.optimal.SurfaceCalc` itself)
        hemi: str, optional
            For which hemisphere to perform the process, `lh`=left hemisphere, `rh`=right hemisphere, `both`=both hemispheres
            (default = `both`)
...
class Neighbours(SurfaceCalc):
    ...
    def find_distance_range(
        ...
        Find the vertices given a range of distance using `vmin` and `vmax`. For instance, if you want the vertices within 2mm
        of your target vertex, specify `vmax=2`. If you want the vertices within 2-4mm, specify `vmin=2,vmax=4`. If vertices
        are found given your criteria, a dictionary collecting key-value pairings between the vertices and their distances to
        ...
        Example
        ----------
        >>> from cxutils.pycortex import Neighbours
        ...
        if hasattr(self, f"{hemi}_dist_to_targ"):
            ...
            if isinstance(distances, np.ndarray):
                ...
                if isinstance(vmin, (int,float)) and not isinstance(vmax, (int,float)):
                    ...
                    raise ValueError("""Not sure what to do. Please specify 'vmin' and/or 'vmax', or use 
                                     self.<hemi>_dist_to_target to fetch all distances""")
...
class pRFCalc():
    ...
    Ideally, the output of these scripts is a numpy array containing the 6 pRF-parameters for each voxel. If you have one of
    those files, specify it in the `prf_file` argument. If, for some reason, you do not have this file, but separate files for
    each pRF variable (e.g., a file for R2, a file for eccentricitry, and a file for polar angle), make sure they are all in 1
    directory and specify that directory in the `prf_dir` parameter. The only function of this class is to collect path names
    and the data arrays containing information about the pRF parameters. It will actually be used in 
    :class:`cxutils.optimal.CalcBestVertex`.

    Parameters
    ----------
    prf_file: str, optional
        Path to a desc-prf_params.pkl file containing a 6xn dataframe representing the 6 variables from the pRF-analysis times
        the amount of TRs (time points). You can either specify this file directly or specify the pRF-directory containing
        separate files for R2, eccentricity, and polar angle if you do not happen to have the prf parameter file.
    subject (str): str, optional
        subject ID as used in SUBJECTS_DIR. If `prf_file` is a BIDS-file and contains 'sub', we'll set `subject` to that.
        Subject ID is required if you want to produce `cortex.Vertex`-objects. In that case, the subject ID must match that of
        FreeSurfer.
    thr: float, optional
    ...
    ----------
    :class:
        Several attributes will be set upon calling the class. These attributes will be necessary to complete
        :class:`cxutils.optimal.CalcBestVertex`
...
class CalcBestVertex():
    ...
    This class actually takes in all attributes from pRFCalc and SurfaceCalc and combines the surface information contained in
    the surface-class with the information from the pRF-class. Specifically, it will call upon
    :class:`cxutils.optimal.SurfaceCalc` and :class:`cxutils.optimal.pRFCalc` as init function giving you complete access to
    anatomical and functional parameter estimates with just calling this class.
    ...
    subject: str
        Subject ID as used in `SUBJECTS_DIR`
    deriv: str, optional
        Path to derivatives folder of the project. Generally should be the path specified with `DIR_DATA_DERIV` in the bash
        environment. This option overwrite the individual specification of `prf_dir`, `cx_dir`, and `fs_dir`, and will look up
        defaults.
    prf_file: str, optional
        Path to a desc-prf_params.pkl file containing a 6xn dataframe representing the 6 variables from the pRF-analysis times
        the amount of TRs (time points). You can either specify this file directly or specify the pRF-directory containing
    ...
    epi_file: str, np.ndarray, bool, optional
        Path to or numpy array containing either time course or averaged BOLD data. Can be used to exclude areas with veins
        (will have low EPI intensities). Format of time course data should be (time,vertices) to ensure averaging over the
        correct axis.
    fs_label: str, optional
        ROI-name to extract the vertex from as per ROIs created with `FreeSurfer`. Default is V1_exvivo.thresh
    use_epi: bool, optional
        try to look in the fMRIprep directory for functional data which we can use to exclude vertices with a low intensity.
        These vertices often denote veins, so we'd like to exclude those when using functional data (e.g., pRF)
        ...
        if self.use_epi:
            ...
            if not isinstance(self.epi_file, (str,np.ndarray)):
                ...
                if isinstance(self.epi_file, str):
                    ...
                    # throw error
                    raise ValueError(f"""Input must be a string point to (time,vertices) file or a (time,vertices) numpy
                                     array, not {type(self.epi_file)}""")
    ...
    def apply_thresholds(
        ...
        Apply thresholds to pRF-parameters and multiply with V1-label mask to obtain only pRFs with certain characteristics in
        V1. The pRF characteristics are embedded in the prf-class, containing the r2, eccentricity, and polar angle values for
        all vertices. Additionally, the surface-class contains structural information regarding thickness and sulcal depth.
        Each of these parameters can be adjusted to find your optimal pRF.

        Parameters
        ----------
        x_thres: int, float, optional
            refers to `x-position` (left-to-right vertical meridian). Usually between -10 and 10. Defaults to 0. A list of
            values for left and right hemisphere should be given. For the right hemisphere, thresholds are treated as `smaller
            than` (left hemifield); for left hemisphere, thresholds denote `greater than` (right hemifield). Used to push the
            vertex away from the fovea
        y_thres: int, float, optional
            refers to y-position in visual space. Usually between -5 and 5 (defaults to the maximum of `y`). Thresholds are
            defined as a range within which a pRF should fall, e.g,. (-5,5) for screen dimensions in y-direction
        r2_thres: int, float, optional
            refers to amount of variance explained. Usually between 0 and 1 (defaults to the minimum `r2`). Threshold is
            specified as 'greater than <value>'
        size_thres: int, float, optional
            refers to size of the pRF in visual space. Usually between 0 and 5 (defaults to 0). Thresholds are defined as a
            range within which a pRF should fall, e.g,. (0.5,2.5)
        ecc_thresh: int, float, optional
            refers to `size` of pRF (smaller = foveal, larger = peripheral). Usually between 0 and 15. Thresholds are defined
            as a range within which a pRF should fall, e.g., (2,3)
        curv_thresh: int, float, optional
            refers to curvature of the cortex. Usually between -1 and 1. Used when `selection=="manual"`, otherwise the
            minimum curvature is specified
        epi thresh: int, float, optional
            refers to the EPI intensities. Defaults to all values, at the risk of selecting areas that might have veins.
            Threshold is specified as 'greater than <value>' and siginifies a percentile value.
        polar_thresh: list, float, optional 
            refers to `polar angle` (upper-to-lower vertical meridian). Usually between -pi and pi. Defaults to -pi. Threshold
            is specified as 'greater than <value>'. A list of values for left and right hemisphere should be given
        thick_thresh: int, float, optional
            refers to cortical thickness as per the `thickness.npz` file created during the importing of a `FreeSurfer`
            subjects into `Pycortex`. The map is defined as NEGATIVE VALUES!! Thicker cortex is represented by a lower
            negative value, usually between -5 and 0. Defaults to the minimum value of the thickness array. Threshold is
            specified as 'lower than <value>'.
        depth_thresh: int, float, optional
            refers to sulcal depth (location of deep/superficial sulci) as per the `sulcaldepth.npz` file created during the
            importing of a `FreeSurfer` subjects into `Pycortex`. Defaults to the minimum value of the sulcaldepth array.
        ...
        if not isinstance(self.selection, str):
            ...
            for i in ['lh', 'rh']:
                ...
                if hasattr(self, f'{i}_prf'):
                    ...
                    if len(val) == 0:
                        print(f"""WARNING [{i}]: Could not find a vertex with these parameters. Try lowering your r2-criteria
                              if used.""")
                        ...
                        if self.srf:
                            ...
                            size_act = utils.select_from_df(
                                self.df_sr,
                                expression=("type = act","&",f"stim_nr = {stim_act_ix}")
                            ).reset_index()["sizes"][0]
                            setattr(self, f"{i}_stim_size_activation", size_act)
                            
                            self.srfs_best_vertices.append(
            ...
            if self.selection == "manual":
                ...
                for tag,hemi,surf in zip(
                    ...
                    if self.srf:
                        ...
                        # find associated stimulus sizes
                        size_suppr = utils.select_from_df(
                            df_suppr,
                            expression=f"stim_nr = {stim_suppr_ix}"
                        ).reset_index()["sizes"][0]
                        setattr(self, f"{hemi}_stim_size_suppression", size_suppr)

                        size_act = utils.select_from_df(
                            df_act,
                            expression=f"stim_nr = {stim_act_ix}"
                        ).reset_index()["sizes"][0]
    ...
    def fetch_normal(self, method="ctx"):
        ...
        Fetch normal vector of best vertex. We can do this by calculating the neighbouring vertices and taking the cross
        product of these vectors (Also see:
        https://sites.google.com/site/dlampetest/python/calculating-normals-of-a-triangle-mesh-using-numpy) [specify
        `methods=="cross"`]. Another option (the default one) is to extract the normal vectors as calculated by
        `Pycortex` itself [specify `method="ctx"`]. Finally, we can apply Newell's calculation of normal vectors
        (https://stackoverflow.com/questions/27326636/calculate-normal-vector-of-a-polygon-newells-method) [specify
        `method="newell"`] 
    ...
    def vertex_to_map(self, concat=True, write_files=False):
        ...
        Create a vertex object that can be read in by pycortex. We can also combine the left/right hemisphere by setting the
        concat flag to True. 'Write_files'-flag indicates that we should write the files to .npy files in the pRF-directory.
        ...
        ----------
        numpy.ndarray
            binarized arrays containing the target vertices in both hemispheres (if `concat==True`), or individual hemispheres
            (if `concat==False`)
        """
    ...
    def to_angle(self, hemi="both"):
        """convert normal vector to angle using :func:`fmriproc.planning.normal2angle`"""
...
class TargetVertex(CalcBestVertex,prf.VertexInfo):

    """TargetVertex

    Full procedure to extract a target vertex given a set of criteria (as per :func:`cxutils.optimal.set_threshold`) from
    structural (:class:`cxutils.optimal.SurfaceCalc`) and functional (:class:`cxutils.optimal.pRFCalc`) by combining
    everything in :class:`cxutils.optimal.CalcBestVert`. The workflow looks as follows:

    * Set thresholds for pRF/structural properties
    * Combine functional/structural properties into :class:`cxutils.optimal.CalcBestVert`
    ...
    * Store pRF/vertex information in `<subject>_desc-prf_params_best_vertices.csv` & `line_pycortex.csv`
    ...
    use_epi: bool
        Allows you to include EPI intensity measures when selecting a vertex; this can be useful to get away from veins.
        Directly specify inputs to use rather than to look for measures in the fmriprep folder (as is the case with just
        use_epi=True). Set `use_epi` to `True` by default
    use_epi: bool
        Allows you to include EPI intensity measures when selecting a vertex; this can be useful to get away from veins.
        Default = False
    webshow: bool
        Start `FreeView` for visual verification. During debugging this is rather cumbersome, so you can turn it off by
        specifying `webshow=False`
    roi: str, optional
        ROI-name to extract the vertex from as per ROIs created with `FreeSurfer`. Default is V1_exvivo.thresh
    use_prf: bool
        In case you want to base the selection of the target_vertex on both structural and functional properties, set
        `use_prf=True`. If you only want to include anatomical information such as curvature, set `use_prf=False`. This is
        relevant for non-visual experiments
    verts: list, optional
    ...
    selection: str, optional
        Method of selection the best vertex. By default `None`, entailing we'll go for vertex surviving all criteria and has
        minimum curvature. Can also be 'manual', meaning we'll open FreeView, allowing you to select a vertex manually
    open_with: str, optional
        if `selection="manual"`, we can either open a viewer with FreeView (defualt, as it open a volumetric view too), but
        can also be `ctx` to open a Pycortex webgl instance
    aparc: bool, optional         
    ...
    ---------
    str
        Creates vertex-information files `<subject>_desc-prf_params_best_vertices.csv` & `line_pycortex.csv` as well as
        figures showing the timecourses/pRF-location of selected vertices.
    
    class
        :class:`cxutils.CalcBestVertex`

    Example
    ----------
    >>> # use parameters to find target vertex for sub-001 in V1_exvivo.thresh by using the default derivatives folders
    ...
    def __init__(
        ...
        if isinstance(self.out, str) and os.path.isfile(self.out):
            ...
            # This thing mainly does everything. See the cxutils/optimal.py file for more information
